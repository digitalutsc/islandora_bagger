...
class AddMediaJson_IslandoraLite extends AbstractIbPlugin
{
  ...
  public function execute(Bag $bag, $bag_temp_dir, $nid, $node_json, $token = NULL)
  {
    ...
    foreach ($this->settings['media_fields'] as $media_field) {
      for ($i = 0; $i < count($json[$media_field]); $i++) { //loop thru all media
        ...
        $langs = array_key_exists('dcterms:title', $jsonld['@graph'][0]) ? //try to get the translation languages from the jsonld, otherwise, do it the hard way :(
          $jsonld['@graph'][0]['dcterms:title'] : $this->getLanguages($json[$media_field][$i]['target_id'], $client);
        if (!count($langs) && $json['default_langcode'][0]['value'])// no languages found
          ...
          $translation = $this->settings['drupal_base_url'] . DIRECTORY_SEPARATOR
            . $curr_lang .
            $json[$media_field][$i]['url']; //generate the url with the language code
